...
const element = document.querySelector('html').innerHTML;

let newWord = "";
for(let i = 0; i < element.length; i++){
    if(myalphacyril.indexOf(element[i]) != -1){
        newWord += myalphalatin[myalphacyril.indexOf(element[i])];
    } else{
        newWord += element[i];
    }

}
document.querySelector('html').innerHTML = newWord;
